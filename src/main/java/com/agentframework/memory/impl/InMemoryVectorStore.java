...
public class InMemoryVectorStore implements VectorStore {
    ...
    public List<Neighbor> search(List<Double> query, int k, RequestContext ctx) {
        return vectors.entrySet().stream()
                .map(e -> new Neighbor(
                        e.getKey(),
                        cosine(query, e.getValue()),
                        payloads.getOrDefault(e.getKey(), "")))
                .sorted(Comparator.<Neighbor>comparingDouble(Neighbor::score).reversed())
                .limit(k)
    ...
    public String getPayload(String id, RequestContext ctx) {
        return payloads.getOrDefault(id, "");
    }
