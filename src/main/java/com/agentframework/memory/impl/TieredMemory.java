...
public class TieredMemory implements Memory {
    ...
    /**
     * Summarizer strategy — implementations can delegate to an LLM or
     * use extractive summarization; the default is a trivial pass-through
     * that preserves the first 500 characters (sufficient for tests and
     * offline scenarios where no LLM is available).
     */
    @FunctionalInterface
    public interface Summarizer {
        /** Produce a concise summary of {@code text}. */
        String summarize(String text);

        /** Default: truncate at 500 chars — suitable for testing only. */
        static Summarizer truncating() {
            return text -> text.length() > 500 ? text.substring(0, 500) + "..." : text;
        }
    }
    ...
    // ── Retrieve (hot + warm; cold fallback when results are insufficient) ─
    public List<MemoryRecord> retrieve(MemoryQuery query, int topK, RequestContext ctx) {
        ...
        List<MemoryRecord> filtered = ranked.stream()
            .filter(r -> r.score() >= query.minScore())
            .limit(topK)
            .collect(Collectors.toList());

        // Cold-tier fallback: if hot+warm results are insufficient, supplement from cold.
        if (filtered.size() < topK) {
            int coldTopK = topK - filtered.size();
            ...
            coldSparse = coldRel.bm25Search(query.naturalLanguage(), coldTopK * 2, ctx);

            Map<String, MemoryRecord> coldCands = new LinkedHashMap<>();
            coldDense .forEach(n -> coldRel.get(n.id(), ctx).ifPresent(r -> {
                if (!seen.contains(r.id())) coldCands.put(r.id(), r.withScore(n.score()));
            }));
            coldSparse.forEach(r -> {
                if (!seen.contains(r.id())) coldCands.putIfAbsent(r.id(), r);
            });

            coldCands.values().stream()
                .filter(r -> r.score() >= query.minScore())
                .sorted(Comparator.comparingDouble(
                    (MemoryRecord r) -> r.score() * (0.5 + 0.5 * scorer.score(r, evalCtx)))
                    .reversed())
                .limit(coldTopK)
    ...
    // ── Update: fall through to warm tier if not found in hot ────────────
    public void update(String id, MemoryContent content, RequestContext ctx) {
        Optional<MemoryRecord> found = hotRel.get(id, ctx);
        if (found.isPresent()) {
            MemoryRecord rec = found.get();
            MemoryRecord updated = new MemoryRecord(id, content, rec.type(), rec.meta(),
                rec.score(), rec.accessCount());
            hotRel.update(id, updated, ctx);
            hotVec.delete(id, ctx);
            hotVec.insert(id, embedFn.embed(content.text()), content.text(), ctx);
            return;
        }
        // Fall through to warm tier if record has aged out of hot.
        warmRel.get(id, ctx).ifPresent(rec -> {
            MemoryRecord updated = new MemoryRecord(id, content, rec.type(), rec.meta(),
                rec.score(), rec.accessCount());
            warmRel.update(id, updated, ctx);
            warmVec.delete(id, ctx);
            warmVec.insert(id, embedFn.embed(content.text()), content.text(), ctx);
        });
    }
    ...
    // ── Consolidate: delegate to Summarizer (LLM-ready hook) ────────────
    public MemorySummary consolidate(String sessionId, RequestContext ctx) {
        ...
        // Delegate to the injected Summarizer — defaults to truncating() for tests;
        // swap in an LLM-backed implementation for production.
        String summary = summarizer.summarize(combined);
        return new MemorySummary(sessionId, summary,
            recs.stream().map(MemoryRecord::id).toList(), Instant.now());
    }

    // ── Janitor: migrate-then-delete to prevent partial-migration ────────
        ...
        new HashMap<>(expirations).forEach((id, exp) -> {
            if (now.isAfter(exp)) {
                hotRel.get(id, sys).ifPresent(rec -> {
                    // Write to cold tier first; only delete from hot/warm after
                    // cold write succeeds to prevent partial-migration data loss.
                    boolean coldWriteOk = false;
                    try {
                        ...
                        LOG.warning("Janitor: cold-write failed for id=" + id +
                            "; hot record retained. Cause: " + e.getMessage());
                    }
                    if (coldWriteOk) {
                        hotVec.delete(id, sys);  hotRel.delete(id, sys);
                        warmVec.delete(id, sys); warmRel.delete(id, sys);
    ...
    public static class Builder {
        ...
        ObjectStore coldObj;
        KnowledgeGraph kg;
        ProspectiveScheduler scheduler;
        ...
        /** Provide an LLM-backed or extractive summarizer for production use. */
        public Builder summarizer(Summarizer s)       { summarizer = s;   return this; }
        ...
        public TieredMemory build() {
            ...
            Objects.requireNonNull(coldObj,   "coldObj");
        ...
        public static TieredMemory inMemory() {
            ...
            return new Builder()
                ...
                .summarizer(Summarizer.truncating())   // swap for LLM-backed in prod
                .embedFn(text -> {
